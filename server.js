...
if (!fs.existsSync(SQLITE_BIN_PATH)) {
    ...
    try {
        ...
        // Check if sqlite3 binary is already in root (seems to happen with some unzip behaviors)
        if (entries.includes('sqlite3')) {
            console.log("Found sqlite3 binary in root.");
            if (!fs.existsSync(SQLITE_BIN_DIR)) fs.mkdirSync(SQLITE_BIN_DIR);
            
            // Move files to sqlite_tools for cleanliness
            ['sqlite3', 'sqldiff', 'sqlite3_analyzer'].forEach(file => {
                if (fs.existsSync(path.join(__dirname, file))) {
                    fs.renameSync(path.join(__dirname, file), path.join(SQLITE_BIN_DIR, file));
                }
            });
        } else {
        ...
        // Cleanup Zip
        if (fs.existsSync(SQLITE_ZIP_PATH)) fs.unlinkSync(SQLITE_ZIP_PATH);
